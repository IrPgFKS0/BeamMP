...
LAN_ITEMS = [
    ...
    slider("defaultCameraDistance", "Default camera distance (LAN)",
           "How far the orbit camera sits from the car, applied whenever you switch into a vehicle or change camera mode -- this is what the zoom keys actually change on the orbit camera, so it works zoomed OUT as well as in. 0 = off (each vehicle's own default, typically ~5 m). The zoom keys still adjust live afterward. Default: 0.",
           0, 30, 0.5, "m"),
    slider("defaultCameraFov", "Default camera FOV (LAN)",
           "Camera field-of-view (lens angle) applied whenever you switch into a vehicle or change camera mode. Lower = narrow/telephoto, higher = wide-angle; the vehicle default is 65. 0 = off (each vehicle's own default). Default: 0.",
           0, 120, 1, "°"),
