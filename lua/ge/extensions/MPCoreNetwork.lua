...
local modVersion = "4.22.2-LAN p13h99" -- POLICY: the base (4.22.2) tracks the upstream BeamMP mod version we are
-- synced against (upstream ca248149 / tag v4.22.2, their 0.39 release); bump it whenever an upstream sync lands so
-- the fork's lineage is readable at a glance. The -LAN pNNhNN suffix is the fork's own build counter.
-- One-line summary of what this build contains; logged at startup next to the version.
local modPatchNote = "p13h99 (pairs combined exe p13h43 -- REBUILT: the Linux client could fail to find BeamNG and quit with a confusing 'cannot get file size: integrity.json'. The launcher searched only the FIRST Steam installation it found, so a machine with both a native and a Flatpak Steam never looked in the second; it also dereferenced a null for a Steam library folder that has no games in it, and a clean 'not found' was reported and then ignored so the real cause was buried. Windows is unaffected. Same fix offered upstream as BeamMP-Launcher#274): mod side is a one-word fix -- positionVE wrote `physmult` without `local`, creating a global in every seated vehicle's Lua VM, which the engine logged with a stack traceback several times per session. p13h98 (pairs combined exe p13h42 -- REBUILT, server-side only; no mod CODE change since h97): a bug-and-regression sweep over everything built since h92 (9 finders, every candidate checked by three independent reviewers) found the predictor GC rework and the h93 turret-aim fix CLEAN, and two defects, both from h94's port of upstream server PR #501. That port ends shutdown with _Exit(0), which by design skips destructors -- and with them (a) the only code that restores a Linux terminal, so Ctrl+C or SIGTERM on the STANDALONE server left the host's shell with no echo and no line editing until 'reset', and (b) the C runtime's stream flush, so anything a Lua plugin left buffered in its own file was discarded. Both now happen just before the exit. The combined host was never affected (the exit is guarded off there). Also fixed: that same _Exit killed the server's own unit-test binary a third of the way through, so 13 of 36 cases had silently not run since h94 while the run still reported success -- now 32 of 36 pass (the other 4 are a pre-existing upstream socket hang on Windows). Finally, h97's note above is corrected: it claimed to be 2-player-verified when no such session had happened, and quoted guard figures from before that harness's own coverage gap was found and closed. p13h97 (pairs combined exe p13h41, UNCHANGED -- mod-only drop-in): GC rework part 3, the remote-ghost predictor. positionVE.updateGFX allocated ~2.8 KB of Lua garbage per RENDER FRAME PER GHOST (44 vec3 + 5 quat) and, unlike the send path, that cost scales with the number of remote players. It now computes into one reusable scratch pool, with the cross-frame snapshots COPIED into dedicated backing vectors (aliasing them to the pool would zero the acceleration damping term every frame, silently) and the force clamp done in place. Every expression is a statement-for-statement transliteration of the original operator form -- nothing re-associated, nothing folded. Proof: an offline guard ran the verbatim old block against the new one over 15 adversarial input streams x 25,000 frames x 5 seeds = 258.7 million bit-pattern comparisons, 0 divergences, with 20 of 20 injected defects caught. updateGFX 55 -> 56 upvalues (LuaJIT fails the whole file at 61). NOTE: the 17-assert smoke gate CANNOT exercise this -- the block is dead code until a second player streams in -- so a 2-player session is REQUIRED before this is considered verified; the gate alone cannot cover it. Also removes a dead lastMailboxVersion local -- the last artifact of upstream updateRemoteData/vehPosPckt receive path, which this fork removed; upstream still uses that name, so a future merge may reintroduce the bare declaration. p13h96 (pairs combined exe p13h41, UNCHANGED -- mod-only drop-in): GC rework part 2, the send path. doSendPosRot allocated ~15 vectors/quaternions per send; it now computes into one reusable pool. MEASURED end to end in a real session: physics-step garbage 220 -> 21 -> 12 bytes per call, ~94%% below where it started, and the worst-case spike 1676 -> 1076 B. What is left is jsonEncode, which cannot go without changing the wire. Position now comes from getPositionXYZ: verified IN-GAME to be bit-identical to getPosition (worst delta 0 over 801 samples at 17 significant digits) because these values go on the wire. Guard-tested: 20,000 random vehicle states, 0.000e+00 divergence on every sent value (pos, vel, rot, rvel). p13h95 (pairs combined exe p13h41, UNCHANGED -- mod-only drop-in): GC rework, hand-ported from upstream PR #952 by stefan750 rather than merged (their hunks target a 438-line positionVE; this fork's is 941 lines). MEASURED in a real session, same probe both sides: physics-step garbage 220 -> 21 bytes per call, about 90%% less, roughly 400 KB/s of allocation removed PER VEHICLE at the 2000 Hz step rate -- that is collector stutter you should feel rather than see in a number. The win is the smoother: vectorSmoothing:get allocated THREE vec3 per call and runs twice per physics step in every vehicle VM; it now computes in place. velocityVE ported in full (force loops ~8 KB/call -> 0). NOT taken from the PR: its setAngularVelocity drops a frame rotation and diverges by up to 5.1 rad/s -- restored here. Both fork changes in velocityVE survive (the findConnectedNodes pcall, the reversed table.remove loops). Guard-tested to 0.000e+00 divergence vs the old code over 600k smoothing steps and 20k random attitudes. p13h94 (pairs combined exe p13h41 -- SERVER fixes, no mod-code change: upstream #454, Util.JsonEncode treated ANY numeric key as an array index, so a plugin table like {[0]='a',[1]=true} encoded as ['a',true] and the 0 key vanished without an error (zero, negative and fractional keys hit the same hole); an array is now strictly integer keys >= 1. Upstream #501, the server could finish shutting every subsystem down and then hang forever on a lingering thread or static destructor -- it now _Exits once the handlers have run, GUARDED so it never fires in combined-host mode, where exiting would take the launcher and your game session down with it. p13h93 (pairs combined exe p13h40 -- REBUILT: the direct vehicle socket sent through a SHARED address struct, so with two receive threads a position/input packet could leave for the wrong vehicle's port; it now stamps a per-call copy. This is the other half of h39's thread-safety fix): plus the turret aim-rate limiter now keys its de-duplication per CONTROLLER instead of per vehicle -- a platform carrying both a gun and a missile turret had one turret's aim updates suppressed by the other's, so that turret never re-aimed on other players' screens. Both found reviewing the h92 code against the 2026-09-05 session logs (that session itself was clean: zero rejected packets, zero module errors, 30 Hz to the end). NOTE for hosts: the grey-ball protection is gated on the 'disableInstabilityPausing' setting and TRUE means the fork handles instabilities -- with it off, BeamNG 0.39.4 can still delete a twice-unstable remote car. p13h92 (pairs combined exe p13h39 -- REBUILT: upstream 4.22.2/3.9.4 sync + #269 async UDP registration; a direct-vehicle-socket thread-safety fix (its registries were touched from 4 threads with no lock -- rare launcher crash); the mod-name security abort now tells the game instead of a stuck Loading; combined host resets readiness if its embedded server exits): integration + bug sweep. MOD: (1) h90's awaiting-resurrection state wrote gameVehicleID=nil where the invariant is -1 -- every frame threw in the nametag loop and the server's removal of that car errored, so it could never be cleaned up; numeric sentinel restored. (2) A remote car you deleted from the player list was mislabeled awaiting-resurrection, which made Restore refuse; it keeps the deleted state (Restore works, deleted blob shows) and clears on respawn. (3) BLOCKER: a remote player's missile/bomb launch with the weapon-cam replayed into YOUR machine -- camera hijack + a queued nil-index into your own car's VM (the sync-killing class); the ghost now always receives cam=off. (4) turret aim target sync sent one packet per FRAME per locked turret over the position lane; now on change + 0.5s keepalive. (5) veReady re-pushes the game speed after a VE reload (slow-mo/pause). (6) plate text after a remote config edit used a nil field -> random plate; fixed. (7) two unguarded GE->VE addKeyEventListener pushes guarded. (8) the settings refill backstop now reads the cloud file too (the nametag keys are cloud-scoped). (9) controller sync no longer forwards -1 for an unmapped target vehicle. p13h91 (pairs combined exe p13h38 -- REBUILT: launcher SECURITY fix, port upstream a9faac2 deceptively titled 'Bump version to v2.8.1' -- validates server-supplied mod filenames are a bare *.zip, closing a path-traversal arbitrary-file-write in mod sync a malicious server could use): plus two p13h82 merge-artifact reverts of the fork's VE-guard convention, both found by a completeness audit. (1) MPPowertrainGE had TWO applyHydroBeams -- the merge kept upstream's UNguarded copy, which shadowed and won, so a hydraulicCylinder vehicle (WL-40 etc.) whose remote VE VM was mid-load would FATAL; duplicate removed, guarded copy restored. (2) positionGE.setGameSpeed push (upstream b2bb5685) went out unguarded -- fires in every vehicle VM on any pause/slow-mo; now if-positionVE guarded. p13h90 (pairs combined exe p13h37, unchanged): grey-ball fix (user-reported: LAN2 stayed a grey ball on LAN1 after free-cam + re-entry). Root cause from the log: their vehicle became an inflated_mat prop, went numerically unstable TWICE on LAN1, and BeamNG 0.39.4's instability flow -- which the fork's old global-override no longer intercepts -- DELETES a twice-unstable vehicle. The object died locally with the sid->id mapping left stale, so every later edit errored 'not found' forever. Fixes, in depth order: (1) the fork now claims instabilities on 0.39.4's sanctioned extension hook (returnData.instabilityHandled) and recovers REMOTE vehicles in place too -- the engine's ghost/teleport/delete ladder never engages in MP; (2) if a remote vehicle's object still dies locally for any reason, onVehicleDestroyed now clears the stale mapping instead of mislabeling it 'removed by local player', so the next edit resurrects it; (3) applyVehEdit heals BOTH dead states (object gone with stale mapping, and spawn-never-received) by clearing the mapping and synthesizing the spawn -- which also REVIVES the p13h81 purple-ball self-heal that had been silently dead since the p13h82 merge (upstream's getGameVehicleID returns -1 where the fork's old one returned nil, so the nil-keyed heal branch never fired). p13h89 (pairs combined exe p13h37, unchanged): sync-overlay truthfulness + LAN options icon. The overlay's 'Pos applied' counter incremented BEFORE the reject guards, so during the p13h82-p13h86 breakage it would have shown a healthy apply rate while 100% of packets were being thrown away -- it now counts only packets that actually passed validation, and rejected packets get their own counter, a red 'Pos REJECTED' overlay row with a persistent [total] tail, a mismatched-builds hint, and a field in the /synclog health line. The header now tells the truth about the send mode ('per-frame' when physics-rate send is off, '+direct' when the direct vehicle socket is on) and Net out notes that direct-socket sends bypass the GE counters. The LAN options section also has its own icon now (signal04a from the game set) instead of the generic gear. p13h88 (pairs combined exe p13h37, unchanged): register the six nametag/blob options-page settings that were never in any defaults registry (fadeVehicles, hideNameTags, nameTagDontFullyHide, nameTagFadeInvert, shortenNametags, spectatorUnifiedColors -- found by the spun-off registration task, ported onto the current tree because its worktree was cut from the pre-4.22 fork point). Matters twice: the game loader DROPS unregistered settings.json keys at boot (the h67 hazard), and the new categorized options tab gates the whole nametag group on hideNameTags === false -- unregistered means undefined, which reads as gate-closed even after the h87 case fix. p13h87 (pairs combined exe p13h37 -- REBUILT: upstream #268 paces the UDP registration burst 10ms apart so the server cannot miss the whole handshake volley; combined mode unaffected, it never sends the burst): upstream 0.39-cleanup sync (#926 + cs/sv translations, 3 commits -- upstream still has nothing else for 0.39.4). Brings: categorized BeamMP options (a six-category family; the LAN items are now their own 'LAN fork' subcategory at the end, injector reworked + still idempotent, refreshIngame pruned from its new home, the playerlistLeftclick forum entry pruned inside its new group), improved pause tab + player-list styling, Chat2 message-delivery fixes, deduplicated UI assets, the dead AngularJS vehicleconfig/optionsOLD folders deleted, and fixOldHUDLayout -- a user's saved pre-0.39 HUD layout gets its app names migrated to the Vue apps. LAN kept: menu trims, Playing-as panel, Direct Connect name field + 30814 default (the fork's own input styling never had upstream's input-box bug). ALSO: release zips moved out of git to GitHub Releases (dist/README links + shas). p13h86 (pairs combined exe p13h36, unchanged): CRITICAL 2-player sync fix (user-reported, log-proven: 13,963 rejected packets in one session). Since the 4.22.1 merge, EVERY remote position packet on the default path was thrown away as 'malformed pose': positionGE.handle kept upstream's call shape (passes the raw JSON string) while the fork's applyPos takes the DECODED table -- string.pos is nil, so the malformed guard rejected everything and remote cars sat frozen. handle now passes the decoded table (and the smoother branch no longer double-decodes). The single-machine smoke gate cannot see this path -- only a 2-player session exercises remote applyPos -- which is exactly why the release gauntlet ends with a real 2-player verify. p13h85 (pairs combined exe p13h36, unchanged): nametag distance separator restored (user-reported). The 4.22.1 merge took upstream's garbage-reduction distance strings, which are TRAILING-space ('42 m ') because upstream separates name and distance with the nametag's own trailing space -- but the fork's #838 cached assembly concatenates the unwrapped nameTagBody straight onto dist, so names rendered as 'Caden42 m'. The separator is now added at the fork-owned assembly line (upstream's format strings stay untouched, keeping future merges clean); also removed the `local unit` that same merge left dead. p13h84 (pairs combined exe p13h36, unchanged): BeamNG 0.39.4 compatibility sweep. The options-layout VFS override was rebuilt on the game's new layout (revision 20260729 -> 20260804: the game dropped its old apply-display button; our stale override would have kept shadowing it) with upstream's beammp category re-inserted byte-identical and all 19 LAN items + prunes re-injected. Everything else verified compatible against a real 0.39.4 boot+join log: API self-check 14/14, zero MP-code errors, all three runtime hook swaps (getDriverData/toggleWalkingMode/onVehicleSwitched) fired, settings loader unchanged (boot-order restore still valid), version gate warn-only as designed. Upstream BeamMP has published NOTHING for 0.39.4 yet (0 new commits on all three repos). Game-side bonus: 0.39.4 fixes the radial-menu spawn-police bug itself (quickAccess now passes the options table setupTrafficWaitForUi expects). p13h83 (pairs combined exe p13h36 -- REBUILT, win + linux): combined-host console parity. The host never saw the BeamMP-mod install line, because deploying the zip to both the server folder and the game at once meant the launcher had nothing to copy and only logged the silent 'already up to date' path; it now always reports WHICH mod build is in the game (sha8 + size) whether it installed one or not. Combined mode also mounted updated SERVER mods at debug level, invisible without --verbose -- that is an info line now, matching what launcher-only mode shows. Fixed the misleading 'Mod caching directory: ./Resources' line in combined mode (the config value prints before the mode override to ./Resources/cache). Full LAN-enhancement sweep after the 4.22.1 merge: 69/69 fork features verified present (static marker sweep + a clean 17/17 gate run with zero MP-code errors in beamng.log). p13h82 (pairs combined exe p13h35, unchanged): FULL UPSTREAM SYNC with BeamMP development (4.22.1, 180 commits since the fork point) -- a real 3-way merge instead of cherry-picks, so future syncs stay cheap. Structure: the GE extension folder is now beammp/ (was multiplayer/) and the extension hooks are namespaced (runPostJoin/onServerLeave -> onBeamMPPostJoin/onBeamMPServerLeave, loadControllerSyncFunctions -> onBeamMPLoadControllerSyncFunctions, trackCamMode -> onBeamMPTrackCameraMode); mp_locales/ is retired for locales/translations/<lang>/beammp/ (new Turkish, real French + Chinese instead of the fork's English placeholders). Menu/UI: upstream server-name text styling (^-codes) + the hover marquee in the server list, the multiplayer pause-menu entry in the in-game top bar, the new Vue HUD apps BeamMP-Chat2 + BeamMP-PlayerList2 alongside the Angular ones, playerlist/session restyle. Settings: the stock MP settings now register from settings/mp_defaults.json (upstream's mechanism) with the fork's h67 restore-from-file rule layered on top, so nametag fade/hide-behind-objects/license-plate-name/player-IDs come with proper defaults. Sync core: string-buffer packet building for Om/Zp/Rc/Xg, per-player vehicle-object cache driving the GE->VE tick loops (with a self-heal rebuild), sim-speed compensation moved into positionVE -- which also fixes it for the #245 direct vehicle socket, that used to bypass the GE-side scaling entirely. Every LAN feature was preserved deliberately: LAN version policy, no-public-list menu trims, Playing-as name + favorite rename, env sync, camera distance, chat dual-emit, weapons/chase, map switching, the VE-guard convention on every GE->VE command, and the fork's own #838 perf work. NOTE: onPreRender is split in two (onPreRenderHousekeeping + onPreRender) because the merged function tripped LuaJIT's 60-upvalue ceiling, which fails the WHOLE file at load. p13h81 (pairs combined exe p13h35, unchanged): 2-player session triage fixes. (1) JOIN WITHOUT A CAR left the MAIN MENU overlay up (user screenshot): setGameState used the pre-4.22 'multiplayer' app layout, which 0.39 no longer resolves -- now 'beammp' (upstream 4.22's rename; we ship beammp.uilayout.json since h59). Both call sites. (2) INVISIBLE REMOTE CAR + MAGENTA BALL diagnosed: the magenta fallback blob = a vehicle whose SPAWN EVENT was lost (registered by a later edit as a permanently-unspawnable husk). Now: the husk logs loudly at creation, the fallback blob logs once per vehicle naming the sid, and -- the real fix -- applyVehEdit SYNTHESIZES the missed spawn from the edit itself (edits carry the same jbm+vcf as spawns; placement injected from the vehicle's live streamed position), so the car appears on the first edit after the loss. If no edit ever comes the ball at least names itself in the log. (3) The name-on-wrong-car report is the same lost-spawn family (activeVehicleID pointing at the unspawned entry); the synthesis restores it. NOTE: 'can't get server id from NNN' at vehicle switch remains a benign pre-registration race (the Om re-send on sid arrival already existed). p13h80 (pairs combined exe p13h35, unchanged): badge '?' flash fixed. The h79 fast tick attached the badge BEFORE the version info arrived from Lua (the menu app's module state resets on play->pause transitions, so the cached version is gone on every Esc) -- it rendered a '?' placeholder until the info event landed, which also read as slower. The badge now NEVER renders without the version: while it's unknown the attach tick pokes MPCoreNetwork.sendBeamMPInfo (throttled to ~1/s) and waits; the badge then appears fully formed, typically within one 350ms tick of the round trip. p13h79 (pairs combined exe p13h35, unchanged): version badge appears promptly after Esc. The bottom-bar badge is DOM-injected and vanishes whenever the menu DOM remounts (opening the pause menu); the re-attach ran on a 2s interval, so the badge lagged in by up to 2s. Badge tick 2000ms -> 350ms, pause-Environment sync-button tick 2000ms -> 700ms (both do one cheap querySelector when nothing to do). p13h78 (pairs combined exe p13h35, unchanged): THE CHAT/SESSION APPS THREW ON EVERY MESSAGE. Found in a user log: 'ReferenceError: DOMPurify is not defined' at BeamMP-Session formatServerName. The mod DOES ship DOMPurify (ui/lib/ext/purify.min.js) but loads it with a fire-and-forget dynamic import that NOTHING AWAITS -- anything formatted before that import resolves throws. formatChatMessage is on the path of EVERY displayed chat line, so one early message killed chat rendering outright (this, together with the h76 missing onBeamMPChatMessage hook, is why the HUD chat showed nothing); formatServerName threw on server-name updates the same way. Both apps now call a never-throw wrapper: the real DOMPurify once the import has landed, and a parser-based sanitizer (drops script/iframe/style/etc, strips on* handlers and javascript:/data: URLs, HTML-escapes as last resort) before that. The import is kept -- the race is fixed, not the library. p13h77 (pairs combined exe p13h35, unchanged): env-sync consent. New 'Allow environment sync from other players (LAN)' checkbox (Options > Multiplayer, default ON so /syncenv keeps working out of the box -- an env change is mild, reversible and sender-attributed, unlike chase consent which stays opt-in). OFF = incoming pushes are ignored with a short toast naming who tried; your own outgoing pushes are unaffected. p13h76 (pairs combined exe p13h35, unchanged): THE HUD CHAT APP RECEIVED NOTHING. UI.chatMessage only triggered the legacy 'chatMessage' guihook (+ the imgui window), but upstream 4.22's BeamMP-Chat app -- the DEFAULT chat, the one most players use -- listens for 'onBeamMPChatMessage' ONLY, and nothing in the mod emitted that name. So every incoming chat line (other players' messages, server lines, and the /? command output) went nowhere on that surface; only the imgui chat showed anything. Now dual-emitted at both branches of UI.chatMessage (same payload table), matching the fork's 0.39 hook-twin policy. Also fixed a latent nil in the same function: the no-player branch passed an undefined global `id` to the imgui chat instead of chatcounter. p13h75 (pairs combined exe p13h35, unchanged): 'Sync environment to players' BUTTON in the pause menu's Environment tab (same one-shot push as /syncenv). Injected into the .pause-environment-digest panel with the version-badge DOM pattern (the game's pause components use scoped CSS, so the button is inline-styled; nav/controller focus not wired -- mouse click). Only appears in an MP session (checked via Lua on each 2s attach tick); click disables it briefly and shows 'Environment pushed.'; receivers get the usual toast + 2s blend. p13h74 (pairs combined exe p13h35, unchanged): camera distance done right (user's design). The slider now sets the orbit camera's OWN defaultDistance (core_camera.setDefaultDistance) in addition to the live distance -- and since the camera's reset path is literally camDist = defaultDistance, vehicle resets restore YOUR distance natively. The h73 deferred re-apply workaround is removed entirely (no onVehicleResetted hook, no onUpdate countdown, no race by construction). Applied on vehicle switch / camera-mode change / live slider moves as before. p13h73 (pairs combined exe p13h35, unchanged): default camera distance vs vehicle reset, round two. The h71 reset hook re-applied the distance IMMEDIATELY on onVehicleResetted, but the camera system restores its own default distance AFTER that hook runs, clobbering ours (user-observed on h71). Now: apply immediately AND re-apply ~0.3s later via a tiny armed-only onUpdate countdown, so ours lands after the camera reset settles. Player vehicle only, as before. p13h72 (pairs combined exe p13h35, unchanged): chat commands actually work from the imgui chat. ROOT CAUSE of '/cmds shows nothing': the imgui chat's send path built and sent the raw 'C:' line itself and never went through UI.chatSend -- where every /command handler lives -- so ALL typed slash commands (incl. /syncenv and the new list) were sent to the server as literal chat text and did nothing locally. The imgui chat now routes through UI.chatSend (which handles commands locally and otherwise sends the identical 'C:' line; raw-send fallback kept if UI is missing). Command list is now /? (aliases /cmds, /help, /commands kept). ALSO (harness, not mod): every smoke-gate run enabled directVehicleSocket+profilePosSync+logSyncStats and h67-persistence kept them on for the next real session -- that is why the profiler/sync-log were 'always on after a new mod version'; the harness now captures the three keys pre-run and restores them by file surgery post-teardown. p13h71 (pairs combined exe p13h35, unchanged): camera-distance polish per user test. (1) The FOV slider is REMOVED from the options panel (distance is the wanted control; defaultCameraFov stays registered-but-inert so an existing settings.json holding it never warns -- syncFullDeformation precedent). (2) Default camera distance now ALSO re-applies on VEHICLE RESET (onVehicleResetted, player vehicle only) -- a reset restores the camera default distance, which un-did the slider until the next switch. (3) On the 'settings not saving' report: the slider value WAS on disk (defaultCameraDistance=10 verified in settings.json); the likely loss window is quitting within seconds of moving a slider -- the game debounces its settings save; a graceful exit flushes (proven by the on-disk value). Re-verify across a restart on this build; /savelogs if it recurs. p13h70 (pairs combined exe p13h35, unchanged): camera-default ZOOM-OUT fix + /cmds. (1) The h69 'Default camera FOV' slider could only zoom IN because on the ORBIT camera the zoom keys change DISTANCE, not FOV -- new 'Default camera distance (LAN)' slider (0-30m, step 0.5, 0 = off; vehicle default ~5m) applied via core_camera.setDistance on the same hooks (vehicle switch / camera-mode change / live slider moves), so the default can sit pulled OUT as well as in. The FOV slider stays for lens-angle preferences (tooltip now explains the difference). (2) /cmds (aliases /help, /commands) lists every chat command locally in the chat window -- /cmds, /maps, /syncenv, /synclog, /savelogs, /mpstate, /netdebug -- never sent to the server or other players. p13h69 (pairs combined exe p13h35, unchanged): TWO LAN FEATURES. (1) /syncenv chat command (alias /envsync): one-shot push of YOUR environment to every other player -- time of day, all cloud parameters, fog, wind, precipitation, gravity, temperature (the game's own core_environment.getState() snapshot); receivers blend to it over 2s (setState lerp) and toast who it came from. Whoever runs the command is the source of truth; no continuous sync afterward. Rides the reliable 'B' relay as a new \"E:\" sub-tag (demuxed in MPWeaponsGE.handle beside fire/chase); old-mod peers ignore it silently, so mixed versions are safe. (2) 'Default camera FOV (LAN)' slider in Options > Multiplayer (0-120 deg, 0 = off/vehicle default): applied on every vehicle switch + camera-mode change + live when the slider moves, so you stop re-zooming after every swap; the zoom keys still adjust live afterward. Both features are pcall-guarded against game-API churn per fork policy. p13h68 (pairs combined exe p13h35, unchanged): pending-fixes fold-in. (1) The 'ResizeObserver loop completed with undelivered notifications' CEF error lines on the servers view are silenced: the filters-rail observer set a max-height INSIDE the container it observes (same-frame layout feedback -- benign but noisy); the callback now defers via requestAnimationFrame. This was upstream 4.22 code ported with the menu. (2) beammpState.js latent init-order bug: the state literal called loadLanPlayerName() before its LAN_PLAYER_NAME_KEY const existed (temporal dead zone); the ReferenceError was swallowed by the function's own try/catch, so the localStorage fast-path for the player name silently returned '' every boot -- masked since h66 by the launcher-name seed, now actually reads the stored name at init (declarations moved above the state literal). (3) UPSTREAM-PRS.md catalog updated with the 0.39-arc inbound triage (notably the partial #838 port + the 3 defects found in that PR, worth reporting upstream). p13h67 (pairs combined exe p13h35, unchanged): MP SETTINGS PERSISTENCE hardened. The game's settings loader drops any settings.json key not in its defaults registry at load time ('Unrecognized setting name' x40 every boot) -- and it loads ~6s before the mod registers its keys. Values usually survive through a later state merge, but that is a boot-order race: any save inside the window rewrites the file without the dropped keys (observed once: settings written by one session were gone after the next). MPConfig now restores the USER'S SAVED value straight from the settings file whenever a registered MP key is nil in the live values (falling back to the default only when the file has nothing), and logs 'mpSettingsRestore: restored N ...' when it kicked in. Verified end-to-end with a two-session probe (tools/mp-smoketest/probe_settings.py): setState -> save -> kill -> reload -> value intact. p13h66 (pairs combined exe p13h35, unchanged): 'Playing as' name fix. The sidebar name (and the Direct Connect prefill + Change-name default) read only the menu's own localStorage, which is empty unless the name was set through THIS menu's dialog on THIS machine -- so it showed 'Player' even though the launcher knew the real name (that's why the in-session player list was right). The launcher's login-check response carries the persisted name (Core.cpp 'Nc' -> Auth username); the menu now seeds from it whenever nothing local is set (mirrored to localStorage, NO Lua push -- it already IS the launcher's name), the sidebar falls back localStorage -> auth.username -> 'Player', and the Direct Connect field back-fills when the name arrives after mount without ever overwriting user input. p13h65 (pairs combined exe p13h35, unchanged): the LAST THREE LAN menu features the Vue port had dropped, found by a pre-release audit of the deleted 0.38 Angular menu (same class of loss as h64's settings). (1) PLAYER NAME is visible and editable again: the sidebar shows 'Playing as <name>' with a Change-name prompt where upstream's account panel used to be (a LAN fork has no BeamMP account, so nothing showed who you are), persisted to localStorage + pushed to MPCoreNetwork.setPlayerName. (2) DIRECT CONNECT regained its name field (blank keeps the current name) -- prefilled with the current name, applied before connecting. (3) FAVORITE RENAME is back: a Rename button on favorited rows stores customName on the favorite (the real sname is left intact) and blank clears it. customName now wins in BOTH the normalizeServer display path AND serverTitleMarkup -- the 0.38 menu had this exact bug (rebuilding the row title from sname alone), which is why renames used to not stick. All three use the game's own Vue popup service (openPrompt), matching what the ported pause-menu cards already use. p13h64 (pairs combined exe p13h35, unchanged): LAN SETTINGS RESTORED + DISCONNECT DIALOG FIX. (1) p13h63's options port shipped upstream's Multiplayer category but DROPPED the LAN fork's own settings (they lived only in the deleted 0.38 Angular page). All of them are back, tooltips carried over verbatim: autoSpawnMode, allowRemoteAIChase, remoteFullProjectiles, physicsRateSend, physRateSendHz, mailboxApplyPos, optimizeMapMarkers, directVehicleSocket, showSyncStats, applyStallDiag, profilePosSync, logSyncStats, profLogFolder + a Save-all-logs button (calls MPConfig.saveLogs directly; the old page's saveLogsAction settings-trigger detour was an Angular scope workaround). Flat list with EXPERIMENTAL/DEBUG label prefixes -- the old showAdvancedMPOptions gate is not replicated (the schema's condition_visible semantics are unverified; a wrong guess would hide the items again). unicycle_pc default-config picker not ported (needs a dynamic options source). tools/inject_lan_options.py is the idempotent injector AND the re-sync tool: on every game patch / upstream sync, refresh the base layout + upstream's category (which carries NEW upstream settings into the LAN branch), then re-run it -- it also re-applies the LAN prunes: refreshIngame REMOVED from the panel (on LAN it makes an in-session list refresh send 'B', which NetReset/TERMINATES the session -- and there is no public list to refresh) and playerlistLeftclick's 'Open player profile' option removed (opens the online forum). (2) 'You have been disconnected' dialog buttons (Continue offline / Return to menu) were unclickable: the old 'showMdDialog' Angular handler (BeamMP-Session app $mdDialog) still renders on 0.39's legacy HUD surface but no longer receives clicks. UI.showMdDialog now emits upstream 4.22's onBeamMPShowVueDialog instead (the ported Vue module shows a native working dialog); deliberately NOT dual-emitted -- the dead Angular overlay would stack on top and block the working one. Server-plugin dialogs (MPGameNetwork.spawnUiDialog) remain Angular-injected and are broken the same way UPSTREAM TOO on 0.39; unused on LAN, left as-is. p13h63 (pairs combined exe p13h35, unchanged): 0.39 MENU FOLLOW-UPS from the first 2-player session. (1) BeamMP SETTINGS restored: 0.39's Options screen is Vue/schema-driven, so the fork's old Angular options page (ui/modules/options, 0.38-era) no longer appeared -- and its options.html override was shadowing the game's own still-shipped copy with stale content. Ported upstream 4.22's answer: a VFS override of ui/ui-vue/src/modules/options/runtime/layout.json that is byte-identical to the installed game's layout (rev 20260720) plus their 47-item Multiplayer category (all keys verified to have live consumers in the fork); the stale Angular options override is deleted. NOTE for future game updates: this override shadows the game's options layout wholesale -- re-diff it against the game copy on every BeamNG patch. (2) MPModManager.getServerMods added (with the serverModDisplayNames bookkeeping + onBeamMPServerModsChanged emits upstream pairs it with, incl. a clear on leave) -- the Vue current-server view and mods card called it through the bridge and errored 'Method not available', leaving the in-session mods list empty. (3) The current-server view's 'server browser' link now lands on Favorites: the public Servers category is empty BY DESIGN on LAN (the launcher answers the 'B' server-list request with an empty list -- and per stock semantics that request also TERMINATES the session, which is why the in-session 'Using cached server list' guard must stay). p13h62 (pairs combined exe p13h35, unchanged): PERF -- Lua garbage cleanup in the per-frame hot paths, hand-ported from upstream PR 838 (open, unmerged; its send-path half is already obsoleted by the fork's direct vehicle sockets). (1) Nametag render loop (MPVehicleGE onPreRender): the prefix/name/suffix/tag string + String() wrapper and the spectator list were rebuilt for EVERY vehicle EVERY frame; now cached per vehicle and rebuilt only when an input changes (nick/role/name setters, settings changes, camera switches -- an activeVehicleID marker also self-heals the guest_<vehicle> flip). Per-frame vec3/quat/Point3F allocs replaced with pooled per-vehicle objects + module scratch (positionGE pools the same pair per packet via the shared mpPooledPos flag; rotation keeps the original quatFromDir(-dir, up) negation that PR 838 itself dropped). Blob colors hex-parsed once (and on settings change) instead of 3x per frame; blob spheres draw via the ffi drawer. (2) Imgui chat (multiplayer/ui/chat.lua + utils.lua): per-segment CalcTextSize now measured once at message-add time (was 2 calls per segment per frame -- ~320B garbage per message per frame on a full chat), name-color ImVec4 pre-converted, style-var/cursor ImVec2/4s replaced with reusable temp vectors, imageButton constant vectors hoisted (style COLORS still read live so the fork's theming options keep working). PR 838's dead recalculateChatHeight machinery was NOT ported. (3) MPGameNetwork connectRetryTimer: declared before disconnectLauncher so its reset writes the local, not a global. p13h61 (pairs combined exe p13h35, unchanged): 0.39 HOTFIX. (1) Removed the dead legacy Angular menu module (ui/modModules/multiplayer) entirely -- 0.39's Vue-only menu router killed its screens, but its BeamMPInfo/authReceived handlers still ran on every hook (incl. each bottom-badge update) and crashed on the missing menu DOM (recurring CEF TypeError 'Cannot set properties of null') -- upstream deleted their copy in 4.22 too; the 0.38 pairing lives in the p13h57 bundle. (2) Fixed upstream's own leftover in BeamMPLoginView: LEGACY_LOGO_PATH pointed at their deleted modModulesOLD folder (a 404 per login-view render); now uses the shipped /ui/assets/BeamMP logo. Known-benign log lines documented: the onUp input-lag warnings are upstream's deliberate 0.39 choice (key-release keeps the chat bind from leaking into the input), the one-time Vue onBeforeUnmount warn + getUIAppsData timing are engine-side noise. p13h60 (pairs combined exe p13h35): 0.39 RELEASE POLISH. Sidebar 'Public Servers' button removed (no public list on a LAN fork; the LAN server lives in Favorites/Recent/Direct Connect -- default landing is now Favorites). Bottom info-bar version badge restored in the 0.38 style (native font/row inside .info-bar-stats, skewed orange divider, right of the game version pill; rebuilt from upstream's last deleted injection design). Rules/TOS screen skipped (LAN policy). Combined exe p13h35 = p13h34 code with an honest pairing banner only. p13h59 (pairs combined exe p13h34, unchanged): 0.39 MENU UI. The game's 0.39 menu router is Vue-only -- the fork's old Angular menu states (menu.multiplayer.*) no longer resolve (clicking Multiplayer logged ui_router 'Route not found: menu.multiplayer.tos' and did nothing). Ported upstream 4.22's ui-vue BeamMP module (ui/ui-vue/mods/BeamMP: server browser, TOS/launcher/login views, direct connect, current-server view) + the new per-language beammp translations + beammp.uilayout.json + their updated BeamMP-Chat app (fork's setCEFFocus deprecation re-applied). Wired to the FORK's Lua: every extensionCommand the Vue state calls already existed in our exports (requestServerList/acceptTos/approveModDownload/login/leaveServer/...); the renamed UI hooks are DUAL-EMITTED (old Angular name + new onBeamMP* twin, 15 sites) so the old in-session Angular surfaces and the new Vue menu both work. The legacy Angular menu button registration is retired (the Vue module registers its own 'BeamMP' button targeting menu.beammp). Packer now ships locales/. p13h58 (pairs combined exe p13h34, unchanged): BEAMNG 0.39 COMPATIBILITY. (1) THE BLOCKER: stock modScript.lua hard-deactivated the whole mod on any game version other than 0.38 -- the moment Steam auto-installed 0.39.1.0, versionCheck killed multiplayerbeammp before any fork code ran and re-deactivated it every boot (defeating the launcher EnableMP re-activation). Now 38+39 are validated and an UNKNOWN future version warns loudly (log+toast) but LOADS ANYWAY -- LAN policy: graceful degradation under the fork's own guards beats silently bricking MP on every game update. (2) imgui: 0.39's dear-imgui requires End()/EndChild() be called UNCONDITIONALLY; restructured all 7 sites (UI.lua chat window + map picker, chat.lua x2, options.lua x2, playerList.lua which was missing EndChild entirely) per upstream #914/#920. (3) unicycle/walking (upstream #918): gameplay_walk.getPlayerUnicycle renamed getCurrentUnicycle (fallback keeps 0.38 compat); new onVehicleSwitched hook deletes our unicycle when entering a vehicle (0.39 no longer routes switches through toggleWalkingMode); restored on leave; MPVehicleGE passenger-cam guard skips unicycle->unicycle. (4) chat input actions fire on onUp (upstream fix). DELIBERATELY NOT PORTED from upstream 4.22 (would break the fork's LAN auth/UI): the entire ui-vue BeamMP module, Vue dialog hook, setGameState beammp layout, directive/layout/translation renames, mp_defaults nameTagFade keys. Upstream's partial-receive fix was already covered by this fork's h15 rewrite. Server/launcher upstreams had NO new commits (p13h34 exe unchanged). Validated: full automated smoke 17/17 on 0.39.1.0 build 20878 (mod load, join, spawn, AI drive, direct-socket ack, edit re-registration). p13h57 (pairs combined exe p13h34): DIRECT VEHICLE SOCKET (#245) + FULL-DEFORMATION REMOVAL. (1) NEW default-off EXPERIMENTAL option 'Direct vehicle socket' (Options>Multiplayer>advanced): own vehicles send POSITION ('Zp') and DRIVER INPUTS ('Vi') straight to the launcher over a dedicated per-vehicle UDP socket (launcherPort+2), bypassing the per-packet queueGameEngineLua string-compile through the single main-thread GE VM -- the measured send-side funnel that grew with vehicle count (profiler-verified: 'GE sendVehiclePosRot' drops to 0 when active). Sends move to the parallel per-vehicle VE VMs; wire format is byte-identical so the server + receivers are unchanged. ONLY latest-wins data rides it -- everything edge/count-based (weapon fire, electrics, powertrain, break groups, controllers) stays on the reliable path. SAFETY: the mod only trusts the socket after the launcher ACKS it (p13h34+ acks each registration + ~1/s keepalive); until then senders keep the normal GE path (brief harmless dups), and with no ack in 3s it reverts with a one-shot log -- so an old launcher/blocked port can never eat a car silently (2-player-test confirmed failure mode). Vehicle EDIT reloads re-register cleanly (the launcher clears its per-sid port pin on a fresh 'Va'). MPInputsVE adds a 2s full-input resync (gated on the socket being active) so a UDP-dropped HELD input (parkingbrake) self-heals. One-shot 'dvSocket' diagnostics in beamng.log trace the lifecycle (require/open/CONFIRMED/revert). (2) FULL-DEFORMATION SYNC REMOVED ENTIRELY (was syncFullDeformation, experimental default-off since p13): even with a fixed transport the intrinsic cost is prohibitive (~100KB jsonEncode of EVERY node+beam at 2Hz per vehicle in VE Lua + a full apply pass on every receiver). getNodes/applyNodes/fullTick + the UI checkbox are gone; 'Xn'/'Xd' packets from old peers are discarded SILENTLY; the setting stays registered but inert so old settings.json files do not warn. Break-group + synced-controller sync are untouched. (3) COMBINED EXE p13h34: hosts the direct-vehicle UDP socket (port+2; 'Va'/'Vd' registration over the core channel; one pinned source port per vehicle; registration ack + keepalive; 'X'-family routed reliable). Old exe + new mod degrades safely to the GE path. p13h56 (pairs combined exe p13h33, unchanged): PROFILING + UI. (1) positionGE.sendVehiclePosRot is now profiled ('GE sendVehiclePosRot' under profilePosSync) -- times the GE-side per-packet send-handler work (jsonDecode the VE payload + simspeed scale + jsonEncode + hand to launcher) for every owned vehicle, i.e. the GE half of the VE->GE send funnel. Prior logs only had 'VE getVehicleRotation' (the cheap VE-side build+enqueue) + nothing for the GE drain; with a 2-player session this now shows the GE-side cost as owned/ghost vehicle count climbs (the #245 baseline). Note: the queueGameEngineLua string COMPILE preceding this handler is engine-internal, invisible to Lua profiling; this captures the handler body incl. the decode/re-encode round-trip. (2) '/synclog' is now ALSO a checkbox: Options>Multiplayer>advanced>DEBUG 'Log sync health to file' -> the same logSyncStats setting the chat command toggles (both work). p13h55 (pairs combined exe p13h33): UPSTREAM PR CHERRY-PICKS. (1) OPT-IN UNRELIABLE (UDP) EVENTS (BeamMP#892/#253/#493): new TriggerServerEventUnreliable(name,data) client-side + MP.TriggerClientEventUnreliable/...JsonUnreliable server-side. Lowercase 'e:' rides UDP (no TCP retransmit / head-of-line blocking) vs reliable 'E:'; on LAN loss ~0 so near-reliable + lower latency. Use for latest-wins/cosmetic data that self-corrects; NOT for incremental/edge data (weapon per-shot, damage) where one drop never self-heals. Works over the combined-host in-memory bridge (rides CombinedServerSendUDP). (2) MOD SYNC additions: transbrake false-value fix (BeamMP#894, `~= nil`); citybus destination-sign sync (BeamMP#884, new 'bus' synced controller); hydraulicCylinder arm/boom beam sync for construction equipment like the WL-40 (BeamMP#880, new MPPowertrainHydrosVE + 'Yh:' powertrain sub-packet) -- inert on vehicles without powertrainHydros, guarded per the fork's cross-module VE-call convention. (3) ROBUSTNESS (combined exe p13h33): server empty-mods.json guard (BeamMP-Server#455, don't parse a 0-byte mods.json); launcher atomic tmp+rename applied to the last download site that still did a direct copy (BeamMP-Launcher#250 -- the fork already had it everywhere else). SKIPPED as already-present or not-applicable: launcher download int-cast (#244, fork already fixed at TCPRcvRaw), --user-path on linux (#223, fork already has it), and the linux Steam-detection robustness PRs (#247/#259 -- LAN2 already detects fine; not worth risking the launch path). p13h52 (pairs combined exe p13h32): FULL-DEFORMATION + WEAPONS-PROJECTILE audit fixes. (1) nodesVE: the syncFullDeformation apply now carries+checks nodeCount/beamCount -- a 2Hz snapshot serialized before the owner EDITED/swapped their vehicle could arrive after the ghost's reload and feed out-of-range cids straight into engine calls (setNodePosition/breakBeam); a mismatched snapshot is stale by definition and is skipped (the next one <=0.5s matches). Fork-internal wire addition (both ends ship in this zip; old packets without counts still apply). (2) MPGameNetwork.sendData FRAME-INTEGRITY rework: a partial send must COMPLETE or the socket must CLOSE -- the 4-byte length header is already on the wire, so the old bounded 1-retry could abandon mid-frame and permanently corrupt the game<->launcher stream framing (realistic trigger: the ~100KB deformation snapshot or a large vehicle-config spawn needing several send() calls). Now: retry while progress; zero-progress allowed a 0.25s window (launcher mid-hitch), then clean disconnect (beats both the pre-fork infinite GE freeze and the bounded-retry corruption). Weapons audit verdict: remoteFullProjectiles chain (MPConfig -> UI -> MPWeaponsGE.handleFire -> uw_fullProjectiles -> fireOneRound lightOnly) verified correct incl. ghost recoil suppression + owner-authoritative blasts; leftover p13h38 uwVeDiag per-shot log in UniversalWeapons.zip flagged for a separate mod repack (not in this repo). p13h51 (pairs combined exe p13h32): UPSTREAM SYNC -- merged BeamMP/BeamMP@c6053fc 'Switched to batch based mod loading' (#893): loadServerMods now mounts synced mods in batches of 5 inside a core_jobsystem job (yielding between batches) instead of all at once -- fixes an upstream-reported BeamNG crash when overloaded by many mod updates and reduces the join-time hitch; directly relevant to this fork's large synced mod set. The fork never modified loadServerMods, so the merge was clean; requestMap still fires after ALL mods are mounted (now a few frames later, async). SERVER: merged BeamMP-Server@176de6b 'BEAMMP_MAX_CONCURRENT_CONNECTIONS env var' (#496) -- per-IP connection-attempt limit configurable via env var; INERT by default (stays 10) and the combined host's virtual client bypasses the accept path, so LAN behavior is unchanged; combined exe rebuilt as p13h32 (Windows; Linux binaries pending a docker rebuild). p13h50 (pairs combined exe p13h31 -- still compatible with p13h32, the exe change is inert): GHOST-FREEZE TRUE ROOT CAUSE -- freeze-after-EDIT flag wipe -- + weapon-vehicle fake-drift metric fix. CORRECTS p13h49's sleep diagnosis: the dual-log pair (host 17:52 + LAN2 17:51) shows every freeze starts at 'applyVehEdit Updating vehicle ...' (the owner swapped vehicle/config: LAN2 froze Dad's ghost at BOTH his 335s Chiron->B25 and 554s B25->ultrabump swaps; the 17:21 'sleep' freeze also began at a 661s edit) and every recovery at an unrelated SETTINGS change. MECHANISM: an edit reloads the ghost's VE VM IN PLACE (same object id) wiping every VE-side flag to default, and applyPos's one-time setup never re-fires (gated on veh.mpVehicleType==nil, which persists on the GE object wrapper) -> with mailboxApplyPos on, GE keeps writing a mailbox the fresh VM never polls (the 17:51 stall log: mailbox version frozen at v1229) = PERMANENT freeze until refreshFlags happens to run. Also hit the OWN car: a reloaded own VM fell back to the VE-default 100Hz SEND_INTERVAL (the relay-overload rate) until the next settings change. FIX: positionVE.onInit announces every VM (re)load to GE (positionGE.veReady) which re-pushes ALL per-vehicle flags (mailbox/profiling/stall-diag/sendHz) and, for remote vehicles, re-arms type 'R' + the anti-sleep (new positionVE.setRemote; the h49 first-packet arm can't fire when the mailbox flag itself was wiped). The h49 anti-sleep stays as defense-in-depth. SECOND FIX (watchdog/overlay honesty): the drift metric compared rxPos to the OOBB CENTER only (p13h27 long-vehicle fix) -- but weapon-mod vehicles that fire NODE projectiles (turretsystem, tank bllt guns) stretch the OOBB hundreds of meters, so a HEALTHY turret ghost applying a steady 30/s read as a constant '170m of drift' on the host overlay, and on LAN2 the watchdog could false-heal a healthy ghost by the OOBB error (throwing it hundreds of meters = a visible warp). Now BOTH anchors (refNode + OOBB center) are measured and the one closer to rxPos is used (long vehicles keep the p13h27 behavior via the OOBB anchor, weapon vehicles get the refNode anchor); 'moved' tracks the refNode (immune to flung nodes); a truly frozen ghost is far on both anchors so detection/heal is unchanged. Overlay drift + /synclog inherit the correction. p13h49 (pairs combined exe p13h31, unchanged): GHOST PHYSICS-SLEEP freeze FIX -- the long-standing intermittent 'apply stalled / recv+0' ghost freeze is ROOT-CAUSED and fixed. Live log (2026-07-04 17:21 host bundle): ghost 0-0 applied 60/s for 5min (peaks ~1m), owner PARKED ~70s, the stationary ghost hit BeamNG's vehicle physics-SLEEP -- the engine stops calling a sleeping vehicle's updateGFX entirely -- and since the mailbox apply is a PULL from updateGFX, the sleeping ghost could never apply again: when the owner drove off, GE kept receiving 10/s for 110s while the ghost VM ran ZERO frames; the watchdog snapped it 24x (24-103m) without ever waking it, until physics happened to rouse it. NOT a p13h48 regression (nothing was removed; the blind spot shipped with mailboxApplyPos in p13) -- the trigger is park-a-while-then-drive-off. FIX (positionVE.setVehiclePosRot): on the FIRST received packet (vehicle guaranteed awake post-spawn) the ghost calls obj:setSleepingEnabled(false) -- the same API BeamNG's own playerController uses -- so a ghost can never doze; re-armed after onReset. Cost: parked ghosts keep simulating (no sleep CPU saving) -- correct behavior for a synced vehicle, matches the pre-mailbox era where the per-packet queueLuaCommand stream kept ghosts awake. ALSO from this log: physRateSendHz=10 sits exactly AT the predictor's 0.1s packetTimeout (updateGFX.stale 2-4/s = the predictor idles between 100ms-apart packets -> steppy ghosts even when healthy). RECOMMENDATION: run 30Hz (the default) on ALL machines; 10Hz is stock-parity but starves the fork's predictor timing. p13h48 (pairs combined exe p13h31, unchanged): SENDER-CLOCK-RESET ghost-freeze fixes -- closes the 'reject+N' stall the p13h43 posApplyStall diagnostic names ('packets REJECTED out-of-order at the tim guard'). (1) positionVE.armSelfSend: the self-send wire clock now resyncs FORWARD-ONLY (max(sendClock, timer)) on a re-arm. The physics thread keeps stepping during a GE hitch, so sendClock runs AHEAD of the frame-driven timer; the old 'sendClock = timer' re-arm after a >0.5s hitch jumped our wire time BACKWARD, making every receiver reject our packets until its clock caught up = our ghost froze on all other machines for the jump duration. (2) positionVE.setVehiclePosRot: a >3s BACKWARD jump in the sender's clock (vehicle Lua reload / clock re-base; same 3s reset rule the GE smoother has always had) now RE-BASES the predictor on the new packet (deltas zeroed, no acc spike; the timeOffset jump guard snaps the time smoother) instead of rejecting every future packet -- previously such a ghost froze until a manual reset, with only the 1Hz watchdog snap papering over it. Small backward steps (real UDP reordering) are still rejected. (3) positionGE.setPing: the launcher's sentinel pings (-1 = no pong yet, -2 = >800ms) were passed through as NEGATIVE ownPing to every vehicle's predictor time-offset; now clamped to 0. p13h47 (pairs combined exe p13h31): REMOVED the p13h42 speed-aware self-heal fast path -- it WARBLED a high-speed car. Live log (h46): the watchdog fired 107x in one session on a fast Bugatti Chiron ghost, every ~0.5s for 15s, snapping it back from 19-27m -- a visible warble. Root: under multi-vehicle load the per-vehicle apply rate dips below the 0.1s packetTimeout (e.g. 58 applied/s / 8 vehicles = ~7Hz = 0.14s gaps), so each ghost briefly FREEZES between applies; harmless at low speed but a hypercar drifts >20m in that 0.3s freeze, and the fast path (heal at >=20m after 0.3s) snapped every one instead of letting the predictor catch up on the next packet. FPS was fine (67-112) so it was NOT an FPS hitch. FIX: the watchdog heals ONLY after a SUSTAINED stall again (>=1.0s + the rxMoved guard) -- the proven-clean p13h41/h44 behavior (0 false heals with the Chiron present). The fast path's only beneficiary was the broken B25 aircraft (now heals ~100m not ~50m, still far better than the 1000m with no watchdog). The residual high-speed stutter is LOAD (trim the live vehicle/AI count), not a snap. p13h46 (pairs combined exe p13h31): sync-stats overlay messages CORRECTED to match the confirmed apply-stall root cause. The 'Pos applied' rate reflects BOTH relay delivery AND local apply throughput (the apply runs per render frame), so a drop can be the relay OR a local FPS hitch. The overlay used to ALWAYS say '>> relay starving: lower send rate' on any apply-rate drop -- wrong when the drop is an FPS hitch (lowering the send rate doesn't help a hitch). FIX (MPDebug): the 'relay starving' hint now only shows when the apply drop is NOT accompanied by an FPS drop (applyBad and not fpsBad); when FPS is also dropping it's a LOCAL hitch and only the FPS-hitch hint shows (trim load). The drift/heals hint now names BOTH causes (load/FPS hitch -> trim mods+AI-traffic; or relay overload -> lower rate; never raise). p13h45 (pairs combined exe p13h30, unchanged): REMOVED fastPredict (the 'Low-GC predictor') entirely -- the function, the toggle, the UI checkbox, the MPConfig setting, the scratch vecs. WHY: a thorough line-by-line audit of updateGFXFast vs the reference updateGFX found NO aliasing/accumulation bug (scratch vecs never leak into persistent state; lastAcc/lastRacc are copied by value; no smoother ever gets a scratch input) -- it was mathematically equivalent. But its GC savings were marginal (~10 scratch vecs while it still allocated quats + the vehVel block + error diffs every frame) and under LuaJIT the allocation-heavy updateGFX is often FASTER (the JIT sinks short-lived allocs), so fastPredict added per-frame overhead + a 160-line parallel predictor that had to mirror updateGFX on every change -- a maintenance footgun that USER-OBSERVED made hitches worse (stock cars hitting the watchdog 'like the B25' with it on, because more hitches = more of the h44 apply-stalls turning visible). The apply-stall ROOT was confirmed (h44, fastPredict off): a ~1s GAME-SIDE FRAME HITCH under heavy load stops the local engine writing the ghost's mailbox (recv+0) -- not netcode. So one correct predictor path now; the lever for the stalls is LOAD (trim mods/traffic), surfaced in the overlay FPS-spike hint. p13h44 (pairs combined exe p13h30, unchanged): the p13h43 apply-stall diagnostic is now a permanent UI TOGGLE instead of always-on. New checkbox Options>Multiplayer>advanced 'Log position apply-stalls' (DEFAULT OFF) -> MPConfig.applyStallDiag -> positionGE.refreshFlags -> positionVE.setApplyStallDiag (live, mirrors fastPredict). Off = no tracking + no logs (the diagnostic is kept, not ripped out). When on it behaves exactly like h43. Also documented (LAN-TUNING.md) WHY the relay caps ~150 pkt/s -- the server's UDPServerMain is a SINGLE thread doing recv -> O(players) locked client lookup -> O(players) fan-out per packet, so it saturates ~150 pkt/s aggregate -- and the trade-offs of raising it (raising the send rate BACKFIRES: backlog->latency->drift->drops; the safe lever is an O(1) client lookup, not more Hz). p13h43 (pairs combined exe p13h30, unchanged): DIAGNOSTIC build to NAME the apply-stall trigger (the thing the self-heal watchdog has been catching). positionVE now instruments its ONE freeze point -- updateGFX's 'no fresh packet' early-return -- and on a sustained stall logs ONE 'posApplyStall' line that distinguishes the cause: recv+0 = packets aren't reaching the VE (GE->VE delivery stalled); reject+N = packets arrive but are rejected out-of-order at the tim guard (sender clock reset on a respawn); realGap small + simSpeed high = the simSpeed-scaled VE clock (dt*localSimspeed, clamped 25) raced past the 0.1s packetTimeout between packets; realGap large = packets genuinely stopped. Purely additive (counters + a one-shot 'W' log, no behavior change); the diag state is one table to stay under LuaJIT's 60-upvalue cap on the hot path. REPRO: fly the B25, let a freeze happen, /savelogs on LAN2 -> grep 'posApplyStall' for the verdict. p13h42 (pairs combined exe p13h30, unchanged): SPEED-AWARE self-heal fast path. The p13h41 watchdog waited a fixed 1.0s before healing a stalled ghost -- fine for a car, but at aircraft speed (~100 m/s) that 1s = ~100m of visible drift before the snap (seen live: a B25Mitchell bomber froze and its ghost ran out to 1038m). FIX (positionGE): a frozen ghost already >= SELFHEAL_FAST_DIST (20m) diverged is healed on FIRST detection instead of waiting the full 1.0s -- a fast sender crosses 20m within one ~0.25s check, so its visible drift is capped ~20-30m; a slow vehicle never reaches 20m in a single check so it still uses the conservative 1.0s path (no new false-heal risk -- moved/rxMoved are 0.25s deltas, so 'frozen-while-sender-moving' is already confirmed, not a 1-frame blip). The B25Mitchell itself is NOT broken -- it's a working aircraft; fast flight is just the worst case for position prediction, which the watchdog now recovers faster. p13h41 (pairs combined exe p13h30, unchanged): RE-ENABLED the self-heal watchdog with a new 'sender-moving' guard. p13h39-40 turned the watchdog fully OFF to stop it fighting the stock predictor on a drifted TANK (the 38-74m warble) -- but that regressed a genuinely STALLED ghost (e.g. a NaN-stuck fullsuv): it would sit out of sync until a manual reset (~10s+, no auto-recovery). FIX (positionGE): the heal now fires ONLY when the ghost is far + FROZEN (its own body barely moved) WHILE fresh positions keep ARRIVING (rxMoved > 0 = the sender is moving and streaming) -- i.e. a real VE-apply stall where the VE isn't applying what the GE is receiving. The tank's transient prediction gaps have rxMoved ~0 (no new packet to apply), so they NEVER qualify -- the warble can't come back. Net: genuine freezes auto-recover in ~1s again; the tank still rides the pure-stock predictor. STALL threshold raised 0.5->1.0s (the rxMoved guard already excludes the tank, so favour fewer false heals). p13h40 (release finalization, pairs combined exe p13h30): (1) launcher [DEBUG] logging is now OFF by DEFAULT (--debug re-enables) -- cleaner logs + a little less CPU/IO on every machine. (2) Removed the broken 'Auto-collect logs when profiling stops' option (autoCollectProfLogs) -- use /savelogs in chat or the 'Save all logs (zip)' button to bundle logs. (3) MPWeaponsGE per-shot SEND/RECV DIAG silenced (DIAG=false). (4) Combined exe: a --debug-gated 'latest-wins drain: coalesced N stale position(s)' log so the client receive-coalescing is verifiable under load. (5) The dead 'Remote sync mode' dropdown + its tracked-hold code are fully REMOVED (UI multiplayer.partial.html + positionGE syncMode/FPS-watcher/pushTrackedHold + MPConfig setting) -- the predictor is pure-stock with no dead toggle. (6) Public docs (RELEASE-NOTES/README-LAN/LAN-TUNING) rewritten to match -- removed watchdog/mode no longer advertised, deep '100 Hz' examples genericized to the configured rate. p13h39: TANK SYNC -- removed the fork's tracked-vehicle predictor BAND-AIDS (hardware testing showed STOCK BeamMP syncs the T-80UD fine with NONE, while the fork's additions made the tank spike 38-74m on the remote at 30Hz). (1) positionVE: the x2 force/ceiling stiffening for >=10-wheel vehicles is OFF (TRACKED_WHEEL_MIN 10->999) -> the tank rides the SAME pure stock predictor force constants as every car. (2) positionGE: the self-heal watchdog SNAP is OFF (SELFHEAL_ENABLED=false) -- it had fired 12x on a drifted tank ghost, snapping it back = the warble; a drifted/frozen ghost is now left to the stock predictor's own teleport. The drift GAUGE (overlay) still runs. So all vehicles, tank included, ride the stock predictor with zero fork interference; the Remote sync mode + setTrackedHold are now inert. (Howitzer per-shot from p13h38 is VALID code -- LuaJIT 2.1 compiles it; its load failures were a torn-file COPY RACE, the launcher re-installing the 78MB UniversalWeapons.zip while BeamNG mounts it. Restart with the mod already cached so the launcher skips the re-copy.) p13h38: ACCURATE per-shot weapon fire RE-ENABLED. The p13h37 diagnostic PROVED the chain is sound: the owner streamed 12 's' shot events, LAN2 received them all ('RECV fire state=s ... vobj=true'), and handleFire set uw_remoteFireShot on the ghost -- so the 'B' relay + per-shot delivery were never the problem (my p13h37 'relay not reaching LAN2' theory was wrong; the p13h36 'nothing fires' was the VE replay side, or the early window where the ghost wasn't found yet, vobj=false). So universalweapons.lua's remote loop is back to firing EXACTLY the streamed shots (uw_remoteFireShot) for slow/gated guns -- no key-press edge-fire, no free-run = no phantom round on a tap -- and ONLY a fast gun (minigun) free-runs on the fire STATE. Added a VE-side 'uwVeDiag' log ('remote replayed N per-shot round(s)') so a /savelogs confirms the ghost actually fires this time. The MPWeaponsGE.DIAG SEND/RECV logs stay on for one more confirming round (set DIAG=false to silence). p13h37: WEAPON-SYNC DIAGNOSTIC build. After p13h36 the remote saw NO projectiles and NO damage from a firing peer (both at once -> the shared 'B' relay is the prime suspect, since fire 's' AND the explosion blast both ride 'B'). TWO changes: (1) reverted the remote fire loop to the pre-per-shot FREE-RUN so weapons WORK again (projectiles + damage; the constant-fire over-firing is back TEMPORARILY) -- this unblocks play while we find the per-shot break; (2) added a temporary MPWeaponsGE.DIAG log on BOTH ends of the 'B' relay: LAN1 logs every SEND (fire code + explosion), LAN2 logs every RECV (handle()/handleFire/explosion incl. parse-fail + vehicle-lookup DROP reasons). Fire one salvo at a target, /savelogs on BOTH, and the SEND-without-a-matching-RECV pinpoints whether 'B' even reaches the remote vs. mis-routes vs. the VE replay. p13h36: weapon fire-rate sync FIX (refines p13h32; UniversalWeapons.zip re-served). The remote still fired a phantom round on the fireweapons KEY-PRESS edge (the owner's BF:1), not only on real shots -- so TAPPING the fire key on a slow/gated gun (the howitzer fires ~1 round / 3s) spawned a projectile per tap on remote screens even when the owner's gun didn't actually fire. Root cause: the remote's free-run '0->1 edge fires one round' path was gated on a per-shot-ping-timing heuristic (perShotActive) that is FALSE on the first tap, before any shot event has arrived. FIX: gate the edge-fire + free-run purely on the gun's STATIC rate (fireInterval < SHOT_SYNC_MIN_INTERVAL = a FAST gun like the minigun, which the owner doesn't per-shot); a per-shot gun (slow/gated) now fires ONLY on the actual broadcast shot events, NEVER on a key edge. p13h35: sync-stats overlay recommendation FIXED -- the 'ghost drifting/correcting' hint said 'cut AI/traffic count or RAISE Position send rate', which is now backwards (raising the rate oversubscribes the relay = MORE drift). Now reads 'cut AI/traffic count or LOWER Position send rate (overload, not too-low a rate)', consistent with the 'relay starving -> lower' hint. Docs (LAN-TUNING.md/README-LAN.md/AGENTS.md) updated with the new sync defaults (30Hz default, 60 ceiling, 100 removed, buffers 256->16) and the 'lower is better' principle. p13h34: REMOVED the 100Hz Position-send-rate option from the in-game UI. 100Hz is above what the single-path relay can carry with 2+ players (~150 pkt/s cap), so it always oversubscribed -> the option was a footgun. UI select is now 60/30/10 (60 = the 2-player ceiling, 30 = default, 10 = stock). positionGE.refreshFlags clamps + MIGRATES any saved value >60 down to 30 (and writes it back, so an existing user who had 100 doesn't keep oversubscribing and the dropdown doesn't show a blank orphaned selection). Read fallbacks (positionGE/MPDebug) default to 30. p13h33: position send-rate DEFAULT 100 -> 30 Hz. Hardware testing showed the fork's remote-car tracking DEGRADED over a session vs the stock mod (stock stays tight). Root cause: the 100Hz default oversubscribed the shared relay (~150 pkt/s total cap, single-threaded UDP forwarder) -- two players each streaming a driven car = ~200 pkt/s, so the relay backlog GREW and every applied remote position got progressively more stale -> the predictor extrapolated further -> growing drift. The predictor INTERPOLATES between updates, so it never needed a high rate (that's why stock's 10Hz looks fine); 100Hz was actively fighting it. 30Hz = ~60 pkt/s for 2 players (under the cap), smoother than stock, no backlog. Tune via Options > Multiplayer > Position send rate (10 = exact stock). p13h32: ACCURATE remote weapon fire RATE. The remote used to free-run UniversalWeapons' own fire loop at the gun's MAX rate for the whole time the owner held fire -- so a turret (which only sets fireweapons=1 when AIMED, gating its real cadence below max) looked like CONSTANT firing on other screens while the source actually fired slower. Now guns at/below ~14 rounds/s (howitzer/m242/50cal) broadcast each ACTUAL shot (reliable 'B' relay, new 's' sub-state) and the remote replays EXACTLY those -> the remote cadence matches the source even when gated. Fast guns (minigun, 20/s) keep the cheap fire-STATE free-run so the relay stays light; a pre-p13h32 owner (no per-shot stream) also falls back to free-run (graceful, via a 0.4s 'is the owner streaming shots?' check). Touches MPWeaponsGE (broadcast + handle the 's' shot, sets uw_remoteFireShot) + universalweapons.lua (owner per-shot broadcast gated by fireInterval >= 0.07s; remote per-shot replay). Re-serves UniversalWeapons.zip. p13h31: sync-health LOG TRAIL. New '/synclog' chat command (toggle, OFF by default) mirrors the sync-stats overlay's line to beamng.log every ~15s ('N synced | drift Xm, M heals | in/out pkt+KB/s | applied/s | FPS') so a /savelogs after an intermittent drift episode captures what the overlay showed over time -- runs even with the overlay hidden. PAIRS WITH combined exe p13h27: server-side mod re-download LOOP DETECTOR -- if a client re-requests the same mod 4+ times in one join (outdated < p13h26 launcher or a stale client cache), the HOST log now prints ONE clear WARN naming the client + mod instead of a silent wall of 'Download ... took Xms' lines. (Earlier this batch: combined exe p13h26 fixed the ACTUAL LAN2 re-download loop -- the client launcher's NewSyncResources had NO success-exit, so a CLEAN verified download re-requested the same mod forever; now it breaks on success with a bounded 3x retry for a genuinely corrupt transfer. p13h25: launcher --no-debug flag. p13h24: server RefreshFiles per client-join so a hot-swapped mod's new hash/size is advertised. Also: turrets.zip re-served fixed -- a repack had put Windows NTFS extras in a Unix (cs=3) zip's LOCAL headers, which BeamNG can't read; stripped them.) p13h30: sync-stats overlay is now DRAGGABLE (removed the NoMove/NoInputs flags; SetNextWindowPos is FirstUseEver so it starts at the top-left but remembers wherever you drag it) and has a darker, more readable backdrop (BgAlpha 0.35 -> 0.7). Drag anywhere on the panel body. PAIRS WITH combined exe p13h25: a new launcher '--no-debug' flag that suppresses [DEBUG] log lines entirely -- it skips both the string build AND the per-line log-file open/write/close in addToLog, shaving a little CPU/IO on a busy host (the launcher's debug() calls are mostly event-driven, so the win is modest, biggest during mod downloads + event bursts). (Turret mod, shipped separately: semi-auto target readout now shows the player's NAME via MPVehicleGE.getNicknameMap() instead of the raw vehicle id, with a #id fallback; plus a DIAG build of missiles.lua that step-logs each missile's lifecycle to pinpoint the 'only the first launches' bug.) p13h29: FINAL pre-public-release robustness sweep (4-agent audit of the mod + combined exe). MOD-side FATAL-class guards so a broken/version-mismatched 3rd-party mod (or a crafted packet) can't kill a vehicle's VE sync VM (the one-way-desync class): (1) controllerSyncVE 'setCameraControlData' rebuilt a quat from variables[1].cameraRotation BEFORE the receive pcall with no nil checks -> now type-guards variables/[1]/cameraRotation first (the one spot the controllerSync pcall didn't cover); (2) MPInputsVE.applyGear ran on the per-frame remote path (NOT pcall'd) and assumed electrics.values.gear is a string and controller.mainController exists -> a custom-powertrain car (gearIndex set, gear nil/no mainController) FATAL-looped its ghost; now guarded; (3) propsControllers spinner engineInfo[6] string-checked to match the hamster path; (4) couplerVE checks getGroupState exists before calling; (5) nodesVE.applyNodes shape-guards its decode (the full-deformation RECEIVE side applies regardless of the local toggle, so a peer with the experimental feature on reaches it); (6) GE-side nil guards: applyVehEdit decode + playerVehicle.config, onServerVehicleResetted pos/rot shape, onServerVehicleCoupled + onVehicleSpawned nil vehicle, applyPos malformed-pose. Position-sync polish: the self-heal watchdog reuses its per-check table (no GE GC litter) and only tracks _diagPeak while profiling. PAIRS WITH combined EXE p13h23: fixes a DeComp() empty-input INFINITE-LOOP HANG (CPU-spin) on both the server and the launcher, reachable by any peer sending a bare 4-byte 'ABG:' packet (empty body -> 0-size buffer -> zlib Z_BUF_ERROR -> grow stays 0 -> never throws); plus the launcher DeComp grow-cap, throwing-fs::file_size hardening in the mod-download paths, and an Options --user-path arg fix. The combined-host leave/rejoin concurrency rewrite was audited CLEAN (no UAF/deadlock/double-client). p13h28: DEFAULT Remote sync mode changed Auto -> SMOOTH (chosen after testing). The firm tracked-vehicle hold that Auto applies at high FPS consistently felt WORSE than the stock-smooth predictor (the tank warble), and Auto's x1<->x2 flipping as FPS bounced added inconsistency. Smooth = pure stock predictor + the frozen-only watchdog = the best default feel. Auto and Accurate stay as options (Options>Multiplayer>advanced) for anyone who wants the firmer hold. (Reminder: heavy sync drift is usually OVERLOAD, not the mode -- keep the live heavy-vehicle count low; the fullsuv is the worst offender.) p13h27: LONG-VEHICLE self-heal FALSE POSITIVE (the 'capsule' 12m bus snapping every 0.5s). Root cause: the sender broadcasts its COG (doSendPosRot = getPosition()+cogRel) so rxPos is the COG, but the self-heal watchdog compared it to veh:getPosition() = the REFNODE. On a long vehicle the COG<->refNode gap is ~6m, which the watchdog read as a permanent '6m off frozen' and snapped the refNode to the COG every 0.5s -- shoving the bus by cogRel each time = visible jitter (looked like a tank/sync issue but was the bus). FIX: the watchdog now compares + snaps against the OOBB CENTER (be:getObjectOOBBCenterXYZ, BeamNG's geometric/COG-proxy center) so it's COG-vs-COG (~0 when synced) at any vehicle length, and the heal offsets the refNode by the refNode->center vector so it no longer shoves long vehicles. Cars unaffected (cogRel <1m). This also makes the overlay 'Ghost drift' number correct for long vehicles. NOTE from the h25 logs: the TANK is now fine (0 self-heals on the host, held x2 in Auto) -- the frozen-only watchdog fix worked; the 157 self-heals were ALL the bus. p13h26: removed the deprecated setCEFFocus(true) call from the Chat/PlayerList/Session UI apps' select() handlers -- BeamNG deprecated it ('doesn't need to be called'; the engine now manages CEF/UI-app focus automatically), so it was just spamming an E-level deprecation warning. Handlers kept as no-ops so the ng-click bindings stay valid. p13h25: SYNC FEEL (why the high-rate fork felt WORSE than stock's 10Hz for the tank). Two of our additions were FIGHTING the stock predictor: (1) the self-heal watchdog HARD-SNAPPED a ghost to the RAW last-received position whenever it was >5m off -- but the stock predictor already teleports to the PREDICTED position, so on a continuously-drifting tank the two yanked it opposite ways ~2x/sec (the warble). FIX: the watchdog is now FROZEN-ONLY -- it only snaps a ghost that's far AND barely moving (a stalled apply); a moving/drifting ghost is left to the stock predictor's own smooth teleport. (2) the tracked-vehicle hold was a fixed x4 (h24) which over-stiffened the tank. FIX: reverted to x2 base and made it a refactor (BASE*multiplier, recomputable, hot paths untouched). NEW 'Remote sync mode' option (Options>Multiplayer>advanced; default Auto): SMOOTH = stock-like (hold x1, frozen-only watchdog -> smoothest), ACCURATE = firm hold x2, AUTO = adapt by local FPS (firm when FPS high, smooth when it drops, hysteresis 42-55fps) -- positionGE pushes the hold to positionVE.setTrackedHold live. Cars are unaffected (never stiffened). If the tank still feels off, try Smooth. p13h24: TANK GHOST DRIFT -- raised the tracked-vehicle position hold from x2 to x4. Diagnosed from a live log: the T-80UD ghost WAS being detected + stiffened ('tracked vehicle (14 wheels): stiffened ... x2') but still drifted ~6-10m (above the 5m self-heal threshold), so the posWatchdog kept snapping it back every ~0.5s (visible warping) -- the tank's tracks were out-muscling the x2 corrective ceiling. x4 should hold it under threshold; if a tank ghost VIBRATES in place, dial TRACKED_HOLD back toward 3.0 (positionVE.lua:~52). The early severe drift (12-18m) was just the window before the detection fires (it waits for wheels to init, ~12s after spawn). NOTE the 'applyVehEdit ... does not correspond' WARNING is NORMAL vehicle-swap handling (it re-spawns the ghost as the new model), not a bug. p13h23: COMMUNITY-RELEASE robustness sweep (multiagent review of the mod + combined exe). FATAL-class hardening so a broken/3rd-party mod in the wild can't kill a vehicle's VE sync VM or the GE receive loop (the one-way-desync class): (1) the top-level GE network dispatch (MPGameNetwork.onUpdate) is now pcall-wrapped -- any error in ANY handler (a broken mod's data, a removed BeamNG API, a malformed packet) is LOGGED instead of breaking that frame's whole receive loop, matching the event-queue path; (2) couplerVE default coupler-RECEIVE path: a peer's trailer/coupler controller that doesn't exist locally was an unguarded getControllerSafe(name).getGroupState() chain (plus a detachGroup() on an empty table) -> FATAL; now resolves a real controller first and skips the branch if absent; (3) controllerSyncVE RECEIVE dispatch is pcall-wrapped, covering EVERY advancedCouplers/general/weapon-mod receive function at once (a controller removed after registration, or a malformed 'variables', no longer FATALs); (4) jsonDecode results nil-guarded in MPPowertrainVE (x2), couplerVE, controllerSyncVE, nodesGE; (5) nodesVE beamstate.beamBroken guarded the same way as beamDeformed; (6) MPVehicleGE spawn handler guards players[ownerID] (nil on a spawn-before-join race). All edits verified parsing under luac 5.3. In-game options/overlays/version strings were audited clean (no dead toggles, overlays report accurate fields). PAIRS WITH combined exe p13h22 (host leave/rejoin gHostLink/gHostClient race + decompression-buffer min/max fix + broken-mod mount log-and-skip) and the new host/ scripts (start-server.bat + pin-cores.ps1 now pin the real BeamNG.drive.x64 process, not the idle loader). p13h22: SECOND FATAL fix in the same syncFullDeformation path -- once p13h21 let getNodes actually SEND, the RECEIVE side (nodesVE.applyNodes) hit beamstate.beamDeformed, which BeamNG 0.3x REMOVED (only the onBeamDeformed callback remains), so the nil call FATAL-killed the receiver's vehicle VM (spammed on a TriX_Chiron). Guarded it; the deformed SHAPE still syncs via setBeamLength, only the (now-absent) damage-system bookkeeping is skipped. syncFullDeformation remains experimental + heavy -- leave it OFF unless you specifically want crash-damage parity. p13h21: (1) FATAL FIX -- nodesVE called a capital 'Round' that is a nil global (the real fn is lowercase 'round', and it was defined AFTER getNodes so out of lexical scope anyway), so every full-deformation-sync tick threw a FATAL Lua error that killed the vehicle's VE VM -- seen crash-looping on a TriX_Chiron. Moved 'round' above getNodes + fixed the case; syncFullDeformation now actually works instead of crash-looping (still gated by the toggle + heavy, leave it off unless you want crash-damage parity). (2) Tracked-vehicle position hold: a driven tank's skid-steer/tracks pushed its ghost off the synced position faster than the predictor's spring caught up, so ONLY the driven tank self-healed (~6m repeatedly) while cars stayed clean. positionVE now detects tracked vehicles by wheel count (>=10; T-80UD has 16, cars 4) and stiffens the correction force/ceiling x2 for them (force/ceiling only, not the gains, to avoid oscillation), once per vehicle, covering both the default and fastPredict paths; wheeled cars untouched. PAIRS WITH the new launcher/combined exe: (a) server DeComp pre-sizes the decompression buffer (16x estimate, clamp 30MB) so a large vehicle config decompresses in one shot instead of the fail-then-retry that logged 'zlib uncompress() failed, trying a larger buffer'; (b) /savelogs in --combined no longer fails with err 123 -- start-server.bat launches via `start` with a relative argv0 so GetEP() returned no dir -> launcherDir empty -> the tar CreateProcessW got an empty lpCurrentDirectory (ERROR_INVALID_NAME); now anchored to the absolute process cwd. (p13h20: a failed/partial vehicle spawn no longer FATAL-crashes the whole multiplayer mod. MPVehicleGE.sendVehicleEdit (run from onVehicleSpawned) indexed nil vehicleData.config when a vehicle failed to load (e.g. a broken mod whose jbeam files couldn't be read -> 'main slot not found') -> a FATAL GE Lua error that killed sync for the ENTIRE session. Now guards nil veh/vehicleData/config and skips the edit broadcast with a warning. (Triggered by a corrupt Tank_T80UD zip: the earlier PowerShell ZipFile.Update repack made a zip bsdtar could read but BeamNG could NOT -- re-packed with bsdtar, the proven method.) (h19: map-switch self-disconnect fix -- the seamless map-switch watchdog gave up after only 60s and force-LEFT the server, but big modded levels load ~90s (ogc_map measured 90.6s), so the HOST doing an in-place switch disconnected its own launcher mid-load ('LAN1 disconnected from itself') while a fresh-joining client (LAN2) was fine (a fresh join gets the new map via the normal handshake, no transition watchdog). Bumped MAP_TRANSITION_TIMEOUT 60->180s (named constant, tunable). (h18: sync-stats overlay now reports the ACTUAL drift symptom. It was staying GREEN while ghosts drifted/corrected because it only watched apply-rate DROPS + FPS -- both look healthy during a traffic FLOOD (high aggregate apply rate, one ghost still starved). NEW 'Ghost drift: X.Xm [N corrections]' row, driven by the positionGE self-heal watchdog's told-vs-actual gap + correction count; reddens when a self-heal fires OR drift exceeds ~8m (well past the few-metre healthy predictor lead). Spawn/teleport transients (>100m) excluded so the number reflects real drift. (h17: traffic-drift/glitch ROOT CAUSE -- h10 claimed non-driven owned vehicles 'drop to the low tick-rate send' but there was NO actual throttle. positionGE.tick (driven at ~FPS by MPUpdatesGE's positionTickrate 1/100) armed only the DRIVEN car to physRateSendHz; EVERY other owned vehicle was sent via getVehicleRotation() on EVERY tick (~FPS 60-90Hz) -- the rate lever never touched it. So N spawned vehicles flooded the relay: the LAN1 log measured 370 pos/s APPLIED at a '10Hz' setting with 7 cars (= 1 driven x10 + 6 AI x~60), starving every ghost into drift, and dialing physRateSendHz down did nothing. FIX: MPUpdatesGE now gates non-driven owned vehicles at a fixed trafficTickrate (12Hz) decoupled from the driven car (which stays full-rate): positionGE.tick(sendTraffic). The driven car can now run 30-60Hz smoothly even with traffic present. The 10Hz no-traffic 'glitch' was just low-rate prediction coarseness -- the predictor was clean in the log (90fps, 0 stale, 0 teleports) -- so run the driven car higher now that traffic no longer floods. (h16: final 3-agent review sweep -- fixed places where earlier nil-guards were one-sided: MPNetworkHelpers 'ready'-branch #nil crash on a pure-timeout partial receive; MPControllerGE.applyControllerData (receive side) + positionGE.sendVehiclePosRot (send side) jsonDecode guards I'd only added on the opposite side. Plus a real CASE bug: MPVehicleGE passed `ServerVehicleString` (capital) to Vehicle:new which reads `serverVehicleString` (lowercase) -> the vehicle got a nil id and every edit-before-spawn was silently dropped. Plus cheap hardening: loginReceived + unknown-role nil-guards, double profiler :stop(), dropped leftover dump() spam (MPNetworkHelpers/MPVehicleGE), FFI header over-alloc (uint32_t[?] count 4->1). PAIRS WITH new launcher+server exes: server FILE-DOWNLOAD path (TCPSendRaw/sendfile) now held under the per-client TCP send-mutex for the whole transfer (completes the C1 desync fix -- it bypassed the mutex), GetPidVid empty-part guard, 'P'-ping stray-NUL, mMods.clear lock-order; launcher WSACleanup on socket-create failure, 'Zp' drop now checks the packet CODE not find()-anywhere, __linux__ typo, crash-history is_directory guard. (h15: base-code review hardening (3-agent audit of the Lua mod + launcher + server). Lua VE-fatal nil-crash fixes that could kill a vehicle's whole sync: getControllerSafe on cars lacking transbrake/lineLock/compressionBrake, unchecked jsonDecode in positionGE/positionVE/MPVehicleGE/MPControllerGE, getOwner/players_vehicle_configs nil on a fast vehicle-edit, a per-frame spectators nil-deref after a player leaves, decoded.vel/rvel + getObjectByID guards. Also: MPNetworkHelpers partial-receive truncation fix (dropped the tail of a split packet), a velocityVE table.remove-in-forward-loop that skipped half the detached nodes, an infinite send-retry loop (MPGameNetwork+MPCoreNetwork) that could freeze the game on a persistent socket error, and a per-send dump() pulled from the controller hot path. PAIRS WITH new launcher+server exes: UDP recv buffers bumped (socket SO_RCVBUF + 64KB per-recv = the drift fix) and a server per-client TCP send-mutex (fixes interleaved-write framing corruption = the one-way desync), plus launcher crash/guard fixes (abort()-on-packet now a graceful drop, uninitialized RegEnumKeyExW buffer, atomic cross-thread flags). OS side: LAN-TUNING.md (Linux net.core.rmem_max is the key item). (h14: sync-stats overlay now flags problems in RED (relay starving / FPS spiking) and tracks peak+persistent severity -- 'Pos applied' and 'FPS' show '[dipped to X, bad Ns total]' that persists after recovery so you can see how deep and how long an issue ran while tuning, with an auto-reset each time you change the send rate (fresh measurement per step) + a red one-line hint of which lever to pull. (h13 tunable position send rate + diagnostic overlay for the 'both-low-CPU, one-drifts-at-a-time' issue (= the shared server relay's throughput, not CPU). NEW UI select 'Position send rate' 100/60/30/10 Hz (live; 10 = stock BeamMP) -> dial down so both players' streams fit a loaded relay, tune back up. Sync-stats overlay now shows 'Pos applied/s' (falls to ~0 when the relay starves -> lower the rate) and 'FPS' (drops with it = one core pegged e.g. the predictor -> try the existing fastPredict toggle). Both levers apply live, no reload. (h12 AI-chase consent rework -- the old 'AI cars chase nearest player' toggle is now 'Allow other players' AI cars to chase me' (opt-in, default OFF): by default nobody else's AI/weapon cars can lock onto you; flip it on to volunteer as a target. Your OWN cars are unchanged (AI radial 'Chase') -- they now target yourself + any remote player who opted in. Opt-in synced via the reliable 'B'/C: relay (no server change for this part). (h11: adaptive remote weapon projectiles -- a weapon car the owner is DRIVING fires full physics projectiles on other screens (broadcastFire tags the owner's active vehicle); a SPAWNED/AI weapon car gets a light muzzle-flash+sound replay (CPU saver, since h10 throttles non-driven cars anyway). New UI toggle 'Full weapon projectiles on remote cars' (default off) forces full everywhere for strong machines. (h10: send-rate budget fix -- ROOT of the no-weapons drift. The relay tops out ~100-150 position pkts/s TOTAL, but physicsRateSend self-sent EVERY owned vehicle at 100Hz; with several owned vehicles (AI traffic) that oversubscribed the budget and starved every ghost into drift. Now only the actively-driven vehicle gets the 100Hz physics-rate send; other owned vehicles drop to the low tick-rate send. (h9: (1) weapon-fire sync per-shot -> fire-STATE (on/off edges + low-rate keepalive): a full-auto burst now costs ~2 packets/sec instead of ~30, fixing the position-sync DRIFT that heavy firing caused by flooding/starving the position stream (remote runs its own fire loop while 'firing'; single shots stay precise via a 0->1 edge that fires one round). (2) self-heal watchdog corrects sooner: 5m/0.5s (was 10m/1.5s) now that h8's gauge reads true divergence. -- h8 FIX watchdog read nametag-clobbered vehicle.position (now rxPos/rxRot)" -- it was reading vehicle.position, which MPVehicleGE's nametag loop overwrites every frame with the rendered OOBB center, so it compared the rendered position to itself (a constant ~height offset per car) and NEVER saw the real drift or fired. Now uses a dedicated rxPos/rxRot copy taken straight from the received packet -> it actually detects a drifted/frozen ghost, the self-heal force-resyncs it, and the 'told-vs-actual peak' instrumentation finally shows true divergence. Also explains 'two map markers far apart for one car': vehicle.position flickers between received and rendered during a drift. (h7 weapon-FIRE sync; h6 drift instrumentation; h5 self-heal watchdog; h4 velocityVE guard+popen; h3 mp_state; h2 savelogs button; h1 explosion-receive be:; base p13 /savelogs+/netdebug+/mpstate; p12 LAN caps, p11 explosion 'B', p10 weapon-chase, p8 electrics/VE hardening))"
-- server
...
local function send(data) -- TODO currently the socket keeps retrying indefinitely if timed out, this freezes the game if the launcher is frozen, breaking the loop with offset the header and break the connection, we could maybe buffer data and try again next frame?
	...
	if error then
		...
		if error == "closed" and launcherConnected then
			...
			if launcherConnected then guihooks.trigger('onBeamMPLauncherConnectionLost') end -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
			launcherConnected = false
			TCPLauncherSocket = nop
			authResult = {}
			guihooks.trigger("authReceived", authResult)
			guihooks.trigger("onBeamMPAuthReceived", authResult) -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
...
local function connectToLauncher(silent)
	...
	if not launcherConnected then
		...
		guihooks.trigger('onBeamMPLauncherConnected') -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	end
end
...
local function isLoggedIn()
	...
	guihooks.trigger('onBeamMPLoginState', loggedIn) -- 0.39 Vue UI twin (their isLoggedIn state hook)
	return loggedIn
end
...
local function sendBeamMPInfo()
	...
	if servers and not tableIsEmpty(servers) then
		...
		guihooks.trigger('onBeamMPServerListReceived', servers) -- server list -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	...
	local info = { -- <players> count on the bottom of the screen
		players = ''..p,
		servers = ''..s,
		beammpGameVer = ''..modVersion,
		beammpLauncherVer = ''..launcherVersion
	}
	guihooks.trigger('BeamMPInfo', info)
	guihooks.trigger('onBeamMPInfo', info) -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
end
...
local function connectToServer(ip, port, name, skipModWarning)
	...
	guihooks.trigger('onBeamMPClearChatHistory') -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
end
...
local function loginReceived(params)
	...
	if (result.success == true or result.Auth == 1) then
		...
		guihooks.trigger('onBeamMPLoggedIn', result.message or '') -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	else
		log('M', 'loginReceived', 'Login failed.')
		loggedIn = false
		guihooks.trigger('LoginError', result.message or '')
		guihooks.trigger('onBeamMPLoginError', result.message or '') -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	end
...
local function handleModWarning(params)
	if params == 'MODS_FOUND' and settings.getValue("skipModSecurityWarning", false) == false and not currentServer.skipModWarning then
		...
		guihooks.trigger('onBeamMPDownloadSecurityPrompt', params)  -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	else 
		send('WY') 
	end
end
...
onLauncherConnected = function()
	...
	extensions.hook('onBeamMPLauncherConnected') -- 4.22 rename twin (upstream namespaced the extension hooks)
	guihooks.trigger('onLauncherConnected')
	guihooks.trigger('onBeamMPLauncherConnected') -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	autoLogin()
...
runPostJoin = function() -- gets called once loaded into a map
	...
	if isMpSession and isGoingMpSession then
		...
		local okGs, gsErr = pcall(core_gamestate.setGameState, 'multiplayer', 'beammp', 'multiplayer') -- 0.39: layout renamed 'beammp' (upstream 4.22)
		if not okGs then log('E', 'runPostJoin', 'setGameState failed (continuing): '..tostring(gsErr)) end
		status = "Playing"
		-- upstream 4.22: register the multiplayer pause menu in the in-game top bar (Esc menu entry)
		if ui_topBar then
		...
		guihooks.trigger('onBeamMPServerJoined') -- 0.39 Vue UI twin (dual-emit keeps the old Angular hooks alive)
	end
end
