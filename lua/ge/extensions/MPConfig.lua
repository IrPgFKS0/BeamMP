...
local defaultSettings = {
	...
	syncFullDeformation = false, -- experimental (LAN): broadcast full node/beam deformation so crash damage matches on all clients. With directVehicleSocket ON the ~100KB snapshots ride the #245 socket as small paced UDP chunks (no GE-thread stall, no TCP head-of-line blocking, incomplete snapshots discarded whole -- see nodesVE/nodesGE 'Xd'); with it OFF the legacy single-blob GE path is used (known to stall the GE thread + starve ghost positions behind it)
	...
	directVehicleSocket = false, -- #245 EXPERIMENTAL (default OFF): own vehicles send POSITION straight to the launcher's direct UDP socket (launcherPort+2), bypassing the VE->GE Lua queue + the GE proxy (the measured send-side funnel). Requires the p13h57+ combined exe. Falls back to the GE path if unavailable. Slow-mo velocity scaling is skipped on the direct path (identical at normal speed). A/B against the profiler ('GE sendVehiclePosRot' should drop toward 0 when on).
