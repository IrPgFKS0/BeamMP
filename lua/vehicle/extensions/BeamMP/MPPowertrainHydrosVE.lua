...
--
-- HydraulicCylinder beam sync (port of BeamMP#880). Syncs the boom/arm hydraulic beam lengths of
-- construction equipment (WL-40 wheel loader, etc.) so a remote ghost's arm/bucket matches the
-- owner's. Inert on vehicles without powertrainHydros (onExtensionLoaded returns early), so it costs
-- nothing on normal cars. Driven from MPPowertrainVE.check() (send side, own vehicle) and applied on
-- the remote ghost's updateGFX via beam actuation.
local M = {}
...
local function getHydroBeams()
    ...
    if foundChangedBeams then
	    obj:queueGameEngineLua("if MPPowertrainGE then MPPowertrainGE.sendHydroBeamData(\'"..jsonEncode(hydroBeamsToSend).."\', "..obj:getID()..") end")
    end
...
local function applyHydroBeams(data)
    ...
    if type(hydros) ~= "table" then return end -- guard a nil/garbage decode (fork convention)
...
local function updateGFX(dt)
    if hasHydroPowertrain and not tickEnabled then
        tickEnabled = true
        if MPPowertrainVE then MPPowertrainVE.setHasHydraulicCylinders() end
    end
